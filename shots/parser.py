                ...
                if line.tokens[0].value == "body":
                    ...
                    for j in reversed(range(len(self.tokenizer.lines))):
                        ...
                        if line.depth == depth and line.tokens[0].value not in js_tags:
                            for j in range(k+1,len(self.tokenizer.lines)):
                                if self.tokenizer.lines[k].depth == 0:
                                    new_line_num = k
                                    break
                            break

                else:
                    for j in range(i+1,len(self.tokenizer.lines)):
                        if self.tokenizer.lines[j].depth == 0:
                            new_line_num = j
                            break
                    break

        if new_line_num == -1:
            new_line_num = len(self.tokenizer.lines)
